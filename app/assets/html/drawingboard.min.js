...
! function() {
	...
	b.prototype.initialize = function(a) {
		this.stack[0] = a, this.initialItem =
		a
	...
	if (this.opts = this.mergeOptions(b), this.ev = new DrawingBoard.Utils.MicroEvent, this.id =
		a, this.$el = $(document.getElementById(a)), !this.$el.length)
		return !1;
	...
	return this.opts.controlsPosition.indexOf("bottom") > -1 ? c += '<div class="drawing-board-controls"></div>' : c = '<div class="drawing-board-controls"></div>' + c, this.$el.addClass("drawing-board").append(c), this.dom = {
		...
		return this.opts.controlsPosition.indexOf(b) > -1 ? (this.dom.$controls.attr("data-align", b), !1) :
		void 0
		...
		history : !1,
		background : !0
	}), this.restoreWebStorage(), this.initDropEvents(),
	void  this.initDrawEvents()) : (this.opts.errorMessage && this.$el.html(this.opts.errorMessage), !1)
	...
	resize : function() {
		...
		var a,
		    b,
		    c = [this.$el.width(), DrawingBoard.Utils.boxBorderWidth(this.$el), DrawingBoard.Utils.boxBorderWidth(this.dom.$canvasWrapper, !0, !0)],
		...
		this.opts.enlargeYourContainer ? ( a = this.$el.width(),
		b = this.$el.height(), this.$el.width(e(c)), this.$el.height(e(d))) : ( a = f(c),
		b = f(d)), this.dom.$canvasWrapper.css("width", a + "px"), this.dom.$canvasWrapper.css("height", b + "px"), this.dom.$canvas.css("width", a + "px"), this.dom.$canvas.css("height", b + "px"), this.canvas.width =
		a, this.canvas.height =
		b
	},
	initControls : function() {
		if (this.controls = [], !this.opts.controls.length || !DrawingBoard.Control)
			return !1;
		for (var a = 0; a < this.opts.controls.length; a++) {
			...
			if ("string" == typeof this.opts.controls[a])
				...
				for (var c in this.opts.controls[a])
				break;
	...
	addControl : function(a, b, c) {
		if ("string" != typeof a && ("object" != typeof a || !a instanceof DrawingBoard.Control))
			return !1;
	...
	setImg : function(a, b) {
		...
		d.onload = function() {
			c.globalCompositeOperation = "source-over", c.clearRect(0, 0, c.canvas.width, c.canvas.height), b.stretch ? c.drawImage(d, 0, 0, c.canvas.width, c.canvas.height) : c.drawImage(d, 0, 0), c.globalCompositeOperation =
	...
	downloadImg : function() {
		var a = this.getImg();
		a = a.replace("image/png", "image/octet-stream"), window.location.href =
		a
	},
	...
	_onCanvasDrop : function(a) {
		...
		if (!b || !b.length || -1 == b[0].type.indexOf("image") || !window.FileReader)
			return !1;
	...
	setColor : function(a) {
		var b = this;
		if ( a = a || this.color, !DrawingBoard.Utils.isColor(a))
			return !1;
	...
	fill : function(a) {
		function b(a) {
			c.data[a[d]] = i, c.data[a[d] + 1] =
			j, c.data[a[d] + 2] =
			k
		}

		if (this.getImg() === this.blankCanvas)
			return this.ctx.clearRect(0, 0, this.ctx.canvas.width, this.ctx.canvas.height), this.ctx.fillStyle = this.color,
			void  this.ctx.fillRect(0, 0, this.ctx.canvas.width, this.ctx.canvas.height);
		...
		if (!DrawingBoard.Utils.compareColors(m, DrawingBoard.Utils.RGBToInt(i, j, k), n)) {
			for (var p,
			    q = [l],
			    r = c.width - 1,
			    s = c.height - 1; p = q.pop(); )
				o && b(p), DrawingBoard.Utils.compareColors(p[g], m, n) && (o || b(p), p[e] > 0 && q.push(DrawingBoard.Utils.pixelAt(c, p[e] - 1, p[f])), p[e] < r && q.push(DrawingBoard.Utils.pixelAt(c, p[e] + 1, p[f])), p[f] > 0 && q.push(DrawingBoard.Utils.pixelAt(c, p[e], p[f] - 1)), p[f] < s && q.push(DrawingBoard.Utils.pixelAt(c, p[e], p[f] + 1)));
	...
	initDrawEvents : function() {
		...
		}, this)), this.dom.$canvas.on("mousemove", $.proxy(function() {
		}, this)), this.dom.$canvas.on("mouseup touchend", $.proxy(function(a) {
	...
	draw : function() {
		...
		if (this.isDrawing) {
			...
			this.ctx.beginPath(), this.ctx.moveTo(b.x, b.y), this.ctx.quadraticCurveTo(this.coords.old.x, this.coords.old.y, this.coords.oldMid.x, this.coords.oldMid.y), this.ctx.stroke(), this.coords.old = this.coords.current, this.coords.oldMid =
			b
		}
	...
	_getInputCoords : function(a) {
		...
		return a.touches && 1 == a.touches.length ? ( b = a.touches[0].pageX,
		c = a.touches[0].pageY) : ( b = a.pageX,
		c = a.pageY), b -= this.dom.$canvas.offset().left, c -= this.dom.$canvas.offset().top, b *= e / d.width, c *= f / d.height, {
			x : b,
			y : c
		}
	},
	...
	_iHasRangeInput : function() {
		var a,
		    ...
		    e = "range";
		return b.setAttribute("type", e),
		a = "text" !== b.type, b.value =
		c, b.style.cssText = "position:absolute;visibility:hidden;", /^range$/.test(e) &&
		void 0 !== b.style.WebkitAppearance && (d.appendChild(b),
		defaultView = document.defaultView,
		a = defaultView.getComputedStyle && "textfield" !== defaultView.getComputedStyle(b, null).WebkitAppearance && 0 !== b.offsetHeight, d.removeChild(b)), !!a
	}
